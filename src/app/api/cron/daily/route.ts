...
// - compares email drafts staff have sent with Ellie's draft (📚 Knowledge → Feedback)
...
export async function GET(request: Request) {
  ...
  const [gmail, instagramToken, sentDrafts] = await Promise.allSettled([
  ...
  return Response.json({ gmail: report(gmail), instagramToken: report(instagramToken), sentDrafts: report(sentDrafts) });
}
